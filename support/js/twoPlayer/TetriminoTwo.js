class TetriminoTwo {
  constructor(name = random(["Z", "S", "J", "L", "T", "O", "I"])) {
    ...
    this.position = createVector(
      int(gameBoard.cols / 4 + gameBoard.cols / 2),
      0
    ); // belongs to p5.js, I set initial position
  }
  ...
  setBackground() {
    this.position = this.spectrum2.position;
    this.moveDown();
  }
  rotate() {
  ...
  paintSpectrum() {
    this.spectrum2 = new TetriminoTwo(this.name);
    this.spectrum2.position = this.position.copy();
    for (let i = 0; i < this.map.length; i++) {
      this.spectrum2.map[i] = this.map[i].copy();
    }
    while (this.spectrum2.moveDown());
    push();
    drawingContext.globalAlpha = 0.3; // var of p5.js
    this.spectrum2.paint();
    pop();
  }
